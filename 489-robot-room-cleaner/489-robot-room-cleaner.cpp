...
public:
    ...
    void dfs(Robot& robot, int curR, int curC, int dir, unordered_map<string, int>& m) {
        
        robot.clean();
        string curLoc = to_string(curR) +'/'+ to_string(curC);
        m[curLoc]++;
        ...
        for (int i=0; i<4; i++) {
            ...
            newLoc = to_string(newR) +'/'+ to_string(newC);
